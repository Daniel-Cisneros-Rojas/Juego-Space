	...
	public class Alien2
	{
		...
		public void mostrar()
		{
			if(colision==0)
			{
			 ...
			 {
			   dibujo.dibujar(5,"alien2.txt",posicion);
			   
			 }
			if(modelo>=30)
			 {
				dibujo.dibujar(5,"alien2_2.txt",posicion);
		...
		{
			if(nave.posicion_disparo_x()>=posicion.x&&nave.posicion_disparo_x()<=posicion.x+(11*5)&&nave.posicion_disparo_y()+3>=posicion.y&&nave.posicion_disparo_y()+3<=posicion.y+(8*5))
			{
				colision=1;
				//dibujo.dibujar(5,"explosion.txt",posicion);
				nave.acerto_disparo();
